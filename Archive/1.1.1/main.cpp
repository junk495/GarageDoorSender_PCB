// =====================================================================================
// GarageAussenSenderESPC3-Lib (code1C) - V1.9 (Sicherer Nachrichtenzähler)
// -------------------------------------------------------------------------------------
// Letzte Änderung: 5. Juli 2025 (Nachrichtenzähler als Replay-Schutz implementiert)
// Hardware:        Waveshare ESP32-C3-Zero
// Funktion:        Ein batteriebetriebener Außensensor für eine Garage.
// Logik:
// - Jede Nachricht enthält nun einen eindeutigen, hochzählenden Zähler, um
//   Replay-Angriffe zu verhindern.
// - Sendet beim ersten Start eine Nachricht mit wakeupCause=0 als Resync-Signal.
// =====================================================================================
...
struct SensorMessage {
  ...
  uint8_t wakeupCause;
  uint32_t messageCounter; // NEU: Zähler für Replay-Schutz
};
...
// RTC-Variablen, die den Tiefschlaf überleben
...
RTC_DATA_ATTR uint32_t messageCounter = 0; // NEU: Globaler Nachrichtenzähler
...
void sendSensorData(uint8_t messageType, uint8_t cause) {
  ...
  #if DEBUG_MODE
  Serial.printf("[SEND] Sensor: T=%.1f, H=%.1f, Tor=%d (%.1f deg), Vbat=%.2f V, I=%.2f mA, Type=%d, Cause=%d, Count=%u\n",
                sensorData.temperature, sensorData.humidity, sensorData.torStatus, z_angle, vbat, current_mA, sensorData.type, sensorData.wakeupCause, sensorData.messageCounter);
  if (vbat < BATTERY_WARNING_VOLTAGE) {
    Serial.println("Akku niedrig! Bitte laden.");
  }
  #endif
...
void sendFingerprintData(int fingerID, uint8_t confidence, uint8_t torStatus, float currentAngle) {
  ...
  #if DEBUG_MODE
  Serial.printf("[SEND] Finger: ID=%d, Tor=%d (Winkel: %.1f deg), ActionID=%d, Vbat=%.2f V, I=%.2f mA, Count=%u\n",
                fingerData.fingerID, fingerData.torStatus, currentAngle, fingerData.actionID,
                fingerData.batteryVoltage, fingerData.batteryCurrent, fingerData.messageCounter);
