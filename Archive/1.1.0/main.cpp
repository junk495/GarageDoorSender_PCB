// =====================================================================================
// GarageAussenSenderESPC3-Lib (code1C) - V1.8 (Weckgrund in Sensor-Nachricht)
// -------------------------------------------------------------------------------------
// Letzte Änderung: 4. Juli 2025 (SensorMessage um 'wakeupCause' erweitert)
...
// - Die Standard-Sensornachricht enthält nun den Grund des Aufwachens.
//   (1=Timer, 2=Fingerprint, 3=ADXL, 0=Power-On/Unbekannt)
...
// - Nach einem Ereignis wird der ADXL-Wakeup für 30s deaktiviert, um Schleifen zu verhindern.
// =====================================================================================
...
struct SensorMessage {
  ...
  uint8_t wakeupCause; // NEU: 1=Timer, 2=Fingerprint, 3=ADXL, 0=Power-On
};
...
void sendSensorData(uint8_t messageType, uint8_t cause) { // NEU: 'cause' Parameter
  float temp = bme.readTemperature();
  float hum = bme.readHumidity();
  ...
  sensorData.wakeupCause = cause; // NEU: Weckgrund setzen

  #if DEBUG_MODE
  Serial.printf("[SEND] Sensor: T=%.1f, H=%.1f, Tor=%d (%.1f deg), Vbat=%.2f V, I=%.2f mA, Type=%d, Cause=%d\n",
                sensorData.temperature, sensorData.humidity, sensorData.torStatus, z_angle, vbat, current_mA, sensorData.type, sensorData.wakeupCause);
  if (vbat < BATTERY_WARNING_VOLTAGE) {
    Serial.println("Akku niedrig! Bitte laden.");
  }
  #endif
  ...
  switch (cause) {
    case ESP_SLEEP_WAKEUP_TIMER: {
      if (deviceState == AWAITING_1_MIN_CHECK) {
        ...
        #endif
        sendSensorData(SENSOR_MESSAGE_TYPE_CONTROL_UPDATE, 1); // Cause 1: Timer
        ...
        readAndPrintSensorData();
        #endif
        sendSensorData(SENSOR_MESSAGE_TYPE_NORMAL, 1); // Cause 1: Timer
        sleep_duration_s = NORMAL_SLEEP_INTERVAL_S;
    ...
    case ESP_SLEEP_WAKEUP_GPIO: {
      ...
      if (gpio_wakeup_status & (1ULL << FP_WAKEUP_PIN)) {
        ...
        #endif
        sendSensorData(SENSOR_MESSAGE_TYPE_NORMAL, 2); // Cause 2: Fingerprint
        ...
        if (fingerID < FINGERPRINT_FOUND) {
          ...
          delay(1000);
          finger.LEDcontrol(FINGERPRINT_LED_OFF, 0, 0, 0);
          sendSensorData(SENSOR_MESSAGE_TYPE_CONTROL_UPDATE, 2); // Cause 2: Fingerprint
        } else {
          #if DEBUG_MODE
        ...
        sendSensorData(SENSOR_MESSAGE_TYPE_CONTROL_UPDATE, 3); // Cause 3: ADXL
      } else {
        #if DEBUG_MODE
        Serial.println("Wakeup Cause: Unknown GPIO Interrupt.");
        #endif
        sendSensorData(SENSOR_MESSAGE_TYPE_NORMAL, 0); // Cause 0: Unknown
      }
      break;
    }
    ...
    default: {
      ...
      deviceState = NORMAL_CYCLE;
      sendSensorData(SENSOR_MESSAGE_TYPE_NORMAL, 0); // Cause 0: Power-On
      sleep_duration_s = NORMAL_SLEEP_INTERVAL_S;
