...
// -------------------------------------------------------------------------------------
// Letzte Änderung: 20. Juni 2025, 19:40
...
#define POST_EVENT_SLEEP_INTERVAL_S 30     // 30 Sekunden
...
void initEspNow() {
  WiFi.mode(WIFI_STA);
  // HINWEIS: Die Sendeleistung kann hier bei der Installation vor Ort angepasst werden.
  // esp_wifi_set_tx_power(40); 
...
// ======================== SETUP ========================
void setup() {
  // ÄNDERUNG 1: CPU-Frequenz reduzieren, um im Wachzustand Strom zu sparen
  ...
  switch (cause) {
    ...
    case ESP_SLEEP_WAKEUP_GPIO: {
      ...
      if (fingerID < FINGERPRINT_FOUND) {
        ...
        delay(1000); // ÄNDERUNG 2: Reduziert von 3000ms
        finger.LEDcontrol(FINGERPRINT_LED_OFF, 0, 0, 0);
