...
// -------------------------------------------------------------------------------------
// Letzte Änderung: 1. Juli 2025 (Hinzufügen von SensorMessage-Typen für differenzierte Übertragung)
...
// - Nach einem Fingerabdruck-Ereignis oder Fehler wird ein zusätzlicher
//   Kontroll-Sensorstatus (SENSOR_MESSAGE_TYPE_CONTROL_UPDATE) nach kurzer Zeit (POST_EVENT_SLEEP_INTERVAL_S) gesendet.
...
#define NORMAL_SLEEP_INTERVAL_S (15 * 60) // 1 Minute (Anpassung für Tests, war 15 * 60)
#define POST_EVENT_SLEEP_INTERVAL_S 30    // 30 Sekunden (vorher 60s)
#define TOR_WINKEL_GESCHLOSSEN 170.0
#define TOR_WINKEL_HALBOFFEN   140.0
#define BATTERY_WARNING_VOLTAGE 3.4       // Vorwarnschwelle

// NEU: Korrekturwerte für BME280-Sensoren
...
#define BME_TEMP_OFFSET_C            0.85  // Korrektur in Grad Celsius
#define BME_HUM_OFFSET_PERCENT       1.73  // Korrektur in Prozentpunkten (RH)

// NEU: Typen für SensorMessage
...
uint8_t receiverMac[] = {0xB0, 0xB2, 0x1C, 0x96, 0xA4, 0x88}; // MAC-Adresse des Empfänger-ESP32
...
// Nötig, da Funktionen aufgerufen werden, bevor sie definiert sind
void initEspNow();
uint8_t calculateTorStatus(float angle);
int getFingerprint();
float getNormalizedZAngle();
void readAndPrintSensorData();
void sendSensorData(uint8_t messageType); // NEU: Parameter für den Nachrichtentyp
...
void readAndPrintSensorData() {
  ...
  // ANPASSUNG: Korrektur der Temperatur und Luftfeuchtigkeit
  ...
  ina219.powerSave(true); // Strom sparen
...
// NEU: messageType Parameter hinzugefügt
void sendSensorData(uint8_t messageType) {
  ...
  sensorData.type = messageType; // <-- NEU: Typ zuweisen
...
void sendFingerprintData(int fingerID, uint8_t confidence, uint8_t torStatus, float currentAngle) {
  ...
  fingerData.type = 99; // FingerEvent Typ ist immer 99
...
void setup() {
  ...
  switch (cause) {
    ...
    case ESP_SLEEP_WAKEUP_GPIO: {
      ...
      #endif
      // NEU: Zuerst eine normale SensorMessage senden, um den allgemeinen Status zu übermitteln
      // Diese Nachricht triggert keinen immediateSend im Empfänger (da Type=NORMAL)
      sendSensorData(SENSOR_MESSAGE_TYPE_NORMAL); // <--- HIER DIE NEUE ZEILE

      int fingerID = getFingerprint(); // Liest den Fingerabdruck
      ...
      if (fingerID < FINGERPRINT_FOUND) {
        ...
        // Bei Fehler: Senden eines Kontroll-Updates (SensorMessage mit Type=CONTROL_UPDATE)
        // Dies triggert immediateSend im Empfänger, da es als wichtiges Update gilt.
        sendSensorData(SENSOR_MESSAGE_TYPE_CONTROL_UPDATE);
      ...
      // Bei Erfolg: Senden der FingerEvent-Nachricht (triggert immediateSend im Empfänger)
