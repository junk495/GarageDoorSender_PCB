...
// Logik:
// - Sendet Sensor-, Torstatus- und Akkudaten alle 15 Minuten.
...
#define POST_EVENT_SLEEP_INTERVAL_S 30    // 60 Sekunden (vorher 30s)
...
void sendSensorData() {
  ...
  #if DEBUG_MODE
  Serial.printf("[SEND] Sensor: T=%.1f, H=%.1f, Tor=%d (%.1f deg), Vbat=%.2f V, I:%.2f mA\n", 
                sensorData.temperature, sensorData.humidity, sensorData.torStatus, z_angle, vbat, current_mA);
  if (vbat < BATTERY_WARNING_VOLTAGE) {
    Serial.println("Akku niedrig! Bitte laden.");
  }
  #endif
...
void sendFingerprintData(int fingerID, uint8_t confidence, uint8_t torStatus, float currentAngle) {
  ...
  #if DEBUG_MODE
  // KORREKTUR: Die printf-Formatierungszeichenkette wurde korrigiert, um den Fehler zu beheben.
...
void setup() {
  ...
  // KORREKTUR: Eine kurze Verzögerung wurde hinzugefügt, um den Sensoren Zeit zur Stabilisierung zu geben.
  delay(100);
  ...
  switch (cause) {
    ...
    case ESP_SLEEP_WAKEUP_GPIO: {
      ...
      if (fingerID < FINGERPRINT_FOUND) {
        ...
        // Selbst bei einem Fehler senden wir einen Status und gehen in den Kontroll-Schlaf
        sendSensorData(); 
    ...
    default:
      ...
      deviceState = NORMAL_CYCLE;
      sendSensorData(); // Sende auch beim ersten Start Daten
      sleep_duration_s = NORMAL_SLEEP_INTERVAL_S;
      break;
  }
  
  #if DEBUG_MODE
  delay(100); // Kurze Pause, damit die letzte serielle Ausgabe gesendet werden kann
