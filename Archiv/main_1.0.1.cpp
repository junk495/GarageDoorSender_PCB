...
// -------------------------------------------------------------------------------------
// Letzte Änderung: 20. Juni 2025, 11:30
...
struct FingerEvent {
  ...
  uint8_t fingerID;
  uint8_t confidence;
  uint16_t actionID; // Wird jetzt dynamisch gesetzt
  uint8_t torStatus;
};
...
// FINGERPRINT_TIMEOUT wird von der Adafruit-Bibliothek definiert.
#define FP_ERROR -3   
...
int getFingerprint() {
  ...
  #endif
  // KORREKTUR: Verwende die Konstante aus der Bibliothek.
  ...
  switch (cause) {
    case ESP_SLEEP_WAKEUP_TIMER: {
      if (deviceState == AWAITING_1_MIN_CHECK) {
        ...
        readAndPrintSensorData(); // KORREKTUR: Aufruf wieder eingefügt
        #endif
        sendSensorData();
